function [X, res, nx, nh, lamb] = Newton(A, b, M, N, k, x0, lamb0, tol, method)
...
for j = 1:k 
    fprintf('Running Newton: the %d-th step ----\n', j);
    rj = A*x-b;
    ...
    while (0.5*sum(Fj_new.^2) >=  0.5*sum(Fj.^2)+c*gamma*delta'*Fj)
        ...
        Fj_new = [lambda_new*Am*rj_new+N_inv*x_new; 0.5*rj_Mn_new-0.5*tau_m];
        if (gamma < 1e-16)
            warning('Stepsize too small. Newton method stopped.')
            X    = X(:,1:j-1);
            res  = res(1:j-1);
            nx   = nx(1:j-1);
            nh   = nh(1:j-1);
    ...
    if(nh(j) < tol)
        disp(['-----------------------------------']);
        disp(['---- Newton method CONVERGED in iteration ',num2str(j), ' ----']);
        disp(['-----------------------------------'])    ;    
        break
    end
...
if(j == k)
    disp(['-----------------------------------']);
    disp(['------- Newton method NOT CONVERGED --------']);
    disp(['-----------------------------------']);
end
