...
[x_dp, alpha_dp] = TikDP(Lm*A, Ln, Lm*b, 1.001);
...
[X1, res1, nx1, nh1, Lamb1] = Lagran(A, b, M, N, k, x0, lamb0, tol, method);
...
er0 = zeros(kk);  % errors of PNT
er1 = zeros(kk);  % errors of Lagrange
er2 = zeros(kk);  % errors of hyb
for i =1:k0
...
hold on;
semilogy(1:kk, er_dp*ones(kk,1), '-','Color',[0.4940 0.1840 0.5560], 'LineWidth', 2.5);
legend('PNT', 'Newton', 'genHyb','Tikh-opt', 'Location', 'northeast','fontsize',15);
...
semilogy(1:k2, Lamb2(1:k2), '-o','Color',[0.3010 0.7450 0.9330],'MarkerIndices',1:1:k2,...
    'MarkerSize',5,'MarkerFaceColor',[0.3010 0.7450 0.9330],'LineWidth',1.3);
hold on;
semilogy(1:kk, lamb_dp*ones(kk,1), '-','Color',[0.4940 0.1840 0.5560], 'LineWidth', 2.5);
xlabel('Iteration','Fontsize',16);
legend('PNT', 'Newton','genHyb', 'Tikh-DP', 'Fontsize',14, 'Location', 'southeast');
...
xlabel('Iteration','Fontsize',16);
legend('PNT, $h(x_{k},\lambda_{k})$','Newton, $h_{w}(x_{k},\lambda_{k})$',...
